...
test.describe("Client Scripts", () => {
	...
	test("window.__atollicScriptRan is true after hydration", async ({
		page,
	}) => {
		// Give the client script a moment to execute
		await page.waitForFunction(
			() => (window as Record<string, unknown>).__atollicScriptRan === true,
			{
				timeout: 5000,
			},
		);
	...
	test("DOM marker element is updated by script", async ({ page }) => {
		await page.waitForFunction(
			() =>
				document.getElementById("script-target")?.getAttribute("data-ran") ===
				"true",
			{ timeout: 5000 },
		);
		await expect(page.getByTestId("script-target")).toHaveAttribute(
			"data-ran",
			"true",
		);
	...
	test("SSR response HTML does not include __atollicScriptRan", async ({
		page,
	}) => {
